...
class Predictor(BasePredictor):
    def setup(self) -> None:
        ...
        model_path = "/src/checkpoints/f5/F5TTS_Base/model_1200000.safetensors"
        self.device = torch.device("cuda" if GPU else "cpu")
        self.f5 = load_model(DiT, model_cfg, model_path, device=self.device)
    ...
    def predict(
        ...
        final_wave, final_sample_rate, _ = infer_process(
            ref_audio_waveform, ref_text_processed, text, self.f5, show_info=print
        )
